...
app.frame('/game', async (c) => {
  ...
  const intents = state.isGameOver
    ? [
        ...
        <Button.Link href={`https://warpcast.com/~/compose?text=${encodeURIComponent(`I just played Tic-Tac-Maxi by POD Play presented by @moxie.eth! ${gameResult === 'win' ? 'I won! 😁' : gameResult === 'lose' ? 'I lost 😔.' : "It's a draw!"} Frame by @goldie & @themrsazon`)}&embeds[]=${encodeURIComponent(`https://podplay.vercel.app/api/shared-game?state=${encodedState}&result=${gameResult}`)}`}>
          Share Results
        </Button.Link>
      ]
